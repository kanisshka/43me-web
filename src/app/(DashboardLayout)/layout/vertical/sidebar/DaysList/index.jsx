  ...
  return (
    <List component="li" disablePadding key={item?.id && item.title}>
      <Link href={`/apps/view-all/${item.id}`}>
        ...
        >
          <ListItemText className="maxWidth">{t(`${item?.dayName}`)}</ListItemText>
          <ListItemText className="maxWidth1">{formattedDate}</ListItemText>
          {/* <Chip
                color='primary'
                size='medium'
                label={t(`${item?.count}`)}
          ...
          <ListItemText>
            <div className="roundCount">{t(`${item?.count}`)}</div>
          </ListItemText>
